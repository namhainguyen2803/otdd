...
def compute_pairwise_distance(list_D, device='cpu', num_projections=10000, evaluate_time=False, **kwargs):
    ...
    for D in list_D:
        X, Y, dict_data = load_full_dataset(data=D, labels_keep=None, maxsamples=None, device=device, precision=dtype, feature_embedding=None, reindex=False, reindex_start=0)
        del X 
        del Y 
        M, C = compute_label_stats(data=D, device=device, eigen_correction="jitter", eigen_correction_scale=1e-4, dtype=dtype) # flatten all data points
