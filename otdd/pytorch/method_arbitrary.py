...
from .utils import generate_uniform_unit_sphere_projections, generate_unit_convolution_projections, generate_uniform_unit_sphere_projections_2, generate_moments, normalizing_moments, normalizing_moments_2
...
class Embeddings_sOTDD():
    ...
    def get_embeddings(self, dict_data, theta, psi, k, factorial_k, use_conv=False):
        proj_matrix_dataset = list()
        for (cls_id, data) in dict_data.items():
            if use_conv is False:
                data = data.reshape(data.shape[0], -1)
            X_projection = self._project_X(X=data, projection_matrix=theta, use_conv=use_conv) # shape == (num_examples, num_projection)
            X_projection = torch.clamp(X_projection, min=-5, max=5)
            avg_moment_X_projection = self._compute_moments_projected_distrbution(X_projection=X_projection, k=k, factorial_k=factorial_k) # shape == (num_projection, num_moments)
...
def compute_pairwise_distance(list_D, device='cpu', num_projections=10000, evaluate_time=False, **kwargs):
    ...
    for i in range(chunk_num_projection):
        chunk_moments = torch.stack([generate_moments(num_moments=num_moments, min_moment=1, max_moment=5, gen_type="uniform") for lz in range(chunk)])
    ...
    for D in list_D:
        X, Y, dict_data = load_full_dataset(data=D, labels_keep=None, maxsamples=None, device=device, precision="float", feature_embedding=None, reindex=False, reindex_start=0)
    ...
    for ch in range(chunk_num_projection):
        ...
        for am in range(len(list_dict_data)):
            chunk_dataset_embeddings = embeddings.get_embeddings(dict_data=list_dict_data[am], 
                                                                ...
                                                                psi=list_psi[ch], 
                                                                k=list_moments[ch], 
                                                                factorial_k=list_factorial_moments[ch],
                                                                use_conv=False)
